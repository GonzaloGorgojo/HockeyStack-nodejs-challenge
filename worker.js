...
const processMeetings = async (domain, hubId, q) => {
  ...
  while (hasMore) {
    ...
    const contactsIds = [...new Set(Object.values(contactAssociations))];

    const contactsInfo =
      (
        await hubspotClient.crm.contacts.batchApi.read({
          inputs: contactsIds.map((id) => ({
            id,
          })),
        })
      )?.results || [];

    const contactsEmail = Object.fromEntries(
      contactsInfo.map((c) => {
        return [c.id, c.properties.email || ""];
      })
    );
    ...
    data.forEach((meeting) => {
      ...
      const meetingProperties = {
        ...
        meeting_started_at: meeting.properties.hs_meeting_start_time,
        meeting_ended_at: meeting.properties.hs_meeting_end_time,
        meeting_attendee_contact: contactsEmail[contactID],
      };
